...
import { Zap, Trophy } from '../components/icons';
import { supabase } from '../lib/supabase';

interface EAFCPageProps {
  onBack: () => void;
}

const EAFCPage: React.FC<EAFCPageProps> = ({ onBack }) => {
  ...
  const loadEAFCProducts = async () => {
    try {
      ...
      // Filtrar apenas produtos de EA FC
      const eafcProducts = (data || []).filter(product => 
      ...
      setProducts(eafcProducts);
    } catch (error) {
      console.error('Erro ao carregar produtos EA FC:', error);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="text-center py-16">
        ...
        <p className="text-xl text-gray-300 max-w-3xl mx-auto px-4">
          Contas com jogadores exclusivos e times completos. Domine os campos com os melhores elencos.
        </p>
      </div>

      {/* Pre-Sale Button */}
      <div className="text-center mb-12">
        <button className="bg-red-600 hover:bg-red-700 text-white font-bold py-4 px-8 rounded-lg transition-all duration-300 transform hover:scale-105 hover:shadow-lg hover:shadow-red-500/25 flex items-center space-x-3 mx-auto">
          <Zap className="w-6 h-6" size={24} />
          <span className="text-lg">PRÉ VENDA LIBERADA</span>
        </button>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
        <div className="bg-gray-900 border border-blue-500 rounded-lg p-4 flex justify-between items-center">
          <span className="text-white">
            {products.length} contas encontradas para ea-fc26
          </span>
      ...
      {products.length === 0 && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center py-16">
          <p className="text-gray-400 text-lg">Nenhuma conta EA FC disponível no momento.</p>
        </div>
      )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16">
       ...
       >
          ...
          <div className="relative">
            <img
              src={product.image_url || 'https://images.pexels.com/photos/274422/pexels-photo-274422.jpeg?auto=compress&cs=tinysrgb&w=400'}
              alt={product.name}
          ...
          <div className="p-6">
            ...
            <div className="text-center text-gray-400 text-sm">
              ID: {product.id} | Categoria: ea-fc26
            </div>
